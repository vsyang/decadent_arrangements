...
import { markAsReadAction, deleteNotificationAction } from "../services/actions";
...
export function NotificationList({ initialNotifications, userRole }: NotificationListProps) {
    const [notifications, setNotifications] = useState<NotificationItem[]>(initialNotifications);
    const [activeTab, setActiveTab] = useState<"all" | "orders" | "promotions" | "account">("all");
            ...
            <div className="space-y-3">
                {filteredNotifications.length === 0 ? (
                    ...
                    <div className="text-center py-12 border border-dashed border-border rounded-lg bg-border/10">
                        <p className="text-muted text-sm">No notifications found in this category.</p>
                    </div>
                ) : (
                    filteredNotifications.map((notif) => (
                        ...
                        >
                            <div className="space-y-1">
                                ...
                                <h3 className={`text-sm font-semibold ${notif.isRead ? "text-muted" : "text-foreground"}`}>
                                    {notif.title}
                                </h3>
