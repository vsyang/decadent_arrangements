...
}: 
{
  ...
  const handleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    startTransition(async () => {
      ...
      } catch (error) { // Por si algo pasa

        setStatus(currentStatus);

        alert(`Something went wrong while updating the status of this order. Error: ${error}`);
    ...
    <div className="relative inline-block w-48">
      <select 
        ...
        className={`w-full appearance-none rounded-lg border-3 py-2 px-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-slate-400 focus:outline-none focus:ring-blue-500 disabled:cursor-not-allowed disabled:opacity-60 ${
          status === "preparing" ? "border-yellow-500 bg-yellow-500/10" :
          status === "delivered" ? "border-green-500 bg-green-500/5" :
          status === "cancelled" ? "border-red-500 bg-red-500/5" : "bg-gray-100/50"}`
        }>
