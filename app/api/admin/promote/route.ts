  ...
  try {
    ...
    if (!match) {
      return NextResponse.json(
        {
          error:
            "Invalid Authorization format. Expected: 'DA <secret> <email>'",
        },
        { status: 400 },
      );
    }
    ...
    if (!session || !session.user || !session.user.email) {
      return NextResponse.json(
        { error: "Active user session not found. Please log in first" },
        { status: 401 },
      );
    }
    ...
    if (sessionEmail !== targetEmail) {
      return NextResponse.json(
        {
          error:
            "Session mismatch. You can only promote your authenticated account",
        },
        { status: 403 },
      );
    }
    ...
    if (!isWhitelisted) {
      return NextResponse.json(
        {
          error:
            "Defensive trigger: This email is not declared within the infrastructure WHITELIST env variable",
        },
        { status: 403 },
      );
    }
    ...
    if (updateResult.length === 0) {
      return NextResponse.json(
        { error: "User account not found in database records" },
        { status: 404 },
      );
    }
    ...
    return NextResponse.json(
      { error: "Internal Server Error during promotion sequence" },
      { status: 500 },
    );
  }
}
